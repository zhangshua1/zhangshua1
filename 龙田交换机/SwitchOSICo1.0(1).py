...
class switch(object):
    ...
    def __connDev(self):
        ...
        # lock = Lock()
        try:
            # lock.acquire()   # 1.1 尝试连接交换机并返回连接对象
    ...
    def __execCommon(self):
        ...
        if connect:
            ...
            # 2.2.2 正则处理获取到的数据
            interface = re.compile(r"GE.*").findall(output)
            ...
            for num in interface:
                ...
                inslist = [interface[insnum],macaddress[insnum],ipaddress[insnum]]
    ...
    def inputExcel(self):
        ...
        if excelFile:
            try:
                ...
                newWorkSheet = newWorkBook.get_sheet()
                # 3.1.3 向新Excel追加写入数据
                sheetRowNum += 1
    ...
    for sw in switchList:
        try:
            ...
            for i in range(num_thread):
                t = threading.Thread(target=ins.inputExcel, kwargs={})
                t.setDaemon(True)
                t.start()
                thread_arr.append(t)
            for i in range(num_thread):
